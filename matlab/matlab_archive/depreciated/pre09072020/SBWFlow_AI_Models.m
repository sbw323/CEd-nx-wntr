%Load Data saved from FlowDataAnalysis.m
load Mymatrix.txt
...
an_free=fopen('Flow_anomaly_free.txt','r');
anomaly_free=textscan(an_free,'%s');
fclose(an_free);
anomaly_free_pipes=zeros(length(anomaly_free{1,:}),1);
for i=1:length(anomaly_free{1,:})
    anomaly_free_pipes(i)=str2double(cell2mat(anomaly_free{1}(i)));
end


%%
%Loading names of reducerd node set generated from FlowDataAnalysis.txt
names_id=fopen('Flow_pipe_names.txt','r');
names=textscan(names_id,'%s');
fclose(names_id);

%%
%Input the flow velocity deviation amplitude
...
keySet = {'NAME','A4D5','A4D4','A4D3','A4D2','A4D1','A3D5','A3D4','A3D3','A3D2','A3D1','A2D5','A2D4','A2D3','A2D2','A2D1','A1D5','A1D4','A1D3','A1D2','A1D1','Red','Orange','Yellow','Green'};
valueSet = {'A1','B1','C1', 'D1', 'E1', 'F1', 'G1' ,'H1', 'I1', 'J1', 'K1', 'L1', 'M1', 'N1', 'O1', 'P1', 'Q1', 'R1', 'S1' ,'T1','U1','V1','W1','X1','Y1'};
mapping_columns=containers.Map(keySet,valueSet);

%Define this as per the number of nodes returned
%from reducer function in FlowDataAnalysis.m
...
%Name of the file to save this data : Output File
...
for k=1:4
    ...
    indici4=indici5;
    %Computing all the indices that repeat atleast for 1 scenario
    ...
    for p=1:5
        ...
        Ysvm_new=zeros(length(Mymatrix),1);
        Ysvm_new(D)=1;
        Xsvm_new = Mymatrix(:,1);
...
%%
%Categorizing into red, oorange, yellow and green
%For SVM, ANN and Actual

%c = confusionmat(red_actual, red_svm);

%c = confusionmat(orange_actual, orange_svm);

%c = confusionmat(yellow_actual, yellow_svm);

c = confusionmat(green_actual, green_svm);
...
writetable(final_table,filename,'Sheet',result_sheet,'Range','B2','WriteRowNames',true);
