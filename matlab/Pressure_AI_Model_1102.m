...
for k=1:4
    ...
    for p=1:5
        ...
        %Train an SVM classifier using the processed data set.
            ...
            newArr_svm=zeros(round(length(label_max)/5),1);
            newArrmax_svm=zeros(round(length(label_max)/5),1);

            jj=1;
            for i=1:5:length(label_max)-5
                if label_max(i)==1 || label_max(i+1)==1 || label_max(i+2)==1  || label_max(i+3)==1  || label_max(i+4)==1 
                    newArrmax_svm(jj)=1;
                end
                jj=jj+1;
            end

            A=table(newArrmax_svm,'VariableNames',...
                {column_name});
            writetable(A,filename,'Sheet',svm_max_sheet,'Range',mapping_columns(column_name));
