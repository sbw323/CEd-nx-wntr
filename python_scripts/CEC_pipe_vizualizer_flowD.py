...
nFile=pd.read_csv(anomalyFreeNode)
...
def flowDeviation_pressure(file0, file1):
    res_arr = file1
...
def midpoint_calculations(input_arr):
...
nodefilename0_11="NYU Anamoly Data_ZeroDeg_Nodes_Leak11.csv"
nodefilename0_21="NYU Anamoly Data_ZeroDeg_Nodes_Leak21.csv"
nodefilename0_31="NYU Anamoly Data_ZeroDeg_Nodes_Leak31.csv"
nodefilename0_41="NYU Anamoly Data_ZeroDeg_Nodes_Leak41.csv"
...
leaknode0_11 = get_file(preDir+nodefilename0_11)
leaknode0_21 = get_file(preDir+nodefilename0_21)
leaknode0_31 = get_file(preDir+nodefilename0_31)
leaknode0_41 = get_file(preDir+nodefilename0_41)

in2_ft2 = 0.0005787
setOfNames = set(leak0_41['NAME'])

fd0_11 = flowDeviation_pressure(nFile,leaknode0_11)
fd0_21 = flowDeviation_pressure(nFile,leaknode0_21)
fd0_31 = flowDeviation_pressure(nFile,leaknode0_31)
fd0_41 = flowDeviation_pressure(nFile,leaknode0_41)

fd0_11 = fd0_11.fillna(value=0.0)
fd0_21 = fd0_21.fillna(value=0.0)
fd0_31 = fd0_31.fillna(value=0.0)
fd0_41 = fd0_41.fillna(value=0.0)

res0_1 = pipe_velocity_calc(nFile1)
normal_vel_1 = pipe_velocity_calc(nFile0)
cal_diff_1 = flowDeviation_vel(normal_vel_1,res0_1)
midpoint_calculations(cal_diff_1)
...
midpoint_calculations(cal_diff_11)
...
midpoint_calculations(cal_diff_21)
...
midpoint_calculations(cal_diff_31)
...
midpoint_calculations(cal_diff)
...
max(fd0_41['PressureDeviation'])

for i, name in enumerate(leaknode0_41.columns):
    print(i+1, name)

def create_graph(graphArr, edgeArr, graphname):
    cntrlnd = '0BEC50B8'
    ...
    for i, j, k in zip(graphArr.NAME,graphArr.NodeXCoordinate,graphArr.NodeYCoordinate):
        pos_dict[i].append(j)
        pos_dict[i].append(k)
    pos_dict_final = dict(pos_dict)

    edgeflow_dict0 = {val:item for val, item in zip(graphArr.NAME, graphArr.FlowDeviation)}

    d3pos_dict = defaultdict(list)

    for d in (pos_dict_final, edgeflow_dict0): # you can list as many input dicts as you want here
        for key, value in d.items():
            d3pos_dict[key].append(value)
    d3pos_dict_final = dict(d3pos_dict)

    node_list = list(graphArr.NAME)
    G.add_nodes_from(pos_dict_final.keys())
    for n in node_list:
        G.node[n]['pos'] = pos_dict_final[n]
        G.node[n]['pressure'] = edgeflow_dict0[n]

    for i, label in enumerate(edgeArr['NAME']):
        pdest = edgeArr['FacilityToNodeName'].iloc[i]
        psource = edgeArr['FacilityFromNodeName'].iloc[i]
        name = edgeArr['NAME'].iloc[i]
        G.add_edge(psource, pdest, n = name)

pos_dict = defaultdict(list)
for i, j, k in zip(leaknode0_41.NAME,leaknode0_41.NodeXCoordinate,leaknode0_41.NodeYCoordinate):
    pos_dict[i].append(j)
    pos_dict[i].append(k)
pos_dict_final = dict(pos_dict)

create_graph(fd0_41, nFile0, 'Severe Leak - Pipe Flow Deviation Normalized')

def draw_3d(graphArr, edgeArr, graphname):
    cntrlnd = '0BEC50B8'
    ...
    edgeflow_dict0 = {val:item for val, item in zip(graphArr.NAME, graphArr.PressureDeviation)}
    ...
    def network_plot_3D(G, angle, save=True):
        ...
        lower = min(graphArr['PressureDeviation'])
        upper = max(graphArr['PressureDeviation'])
        tp_arr = (graphArr['PressureDeviation']-lower)/(upper-lower)
        ...
        with plt.style.context(('ggplot')):
            ...
            ax.set_zlabel(graphname, fontsize=30)
            ax.set_zlim(lower,upper)

            ctr=0
            for key, value in d3pos_dict_final.items():
                xi = value[0][0]
                yi = value[0][1]
                zi = value[1]
            ...
            for i in G.edges:
                x = np.array((d3pos_dict_final[i[0]][0][0],d3pos_dict_final[i[1]][0][0] ))
                y = np.array((d3pos_dict_final[i[0]][0][1], d3pos_dict_final[i[1]][0][1]))
                z = np.array((d3pos_dict_final[i[0]][1], d3pos_dict_final[i[1]][1]))
        ...
        if save is not False:
            plt.savefig("/Users/aya/Documents/code-pfs/gas-nx/plots/03302020--latestVel/Leak_31_ZeroDegrees_pressuredeviation.png")
            plt.show()
        else:
            plt.show()
...
draw_3d(fd0_41, nFile0, 'Severe Leak - Node Pressure Deviation Normalized')

draw_3d(fd0_11, nFile0, 'Low Leak - Node Pressure Deviation Normalized')

draw_3d(fd0_21, nFile0, 'Moderate Leak - Node Pressure Deviation Normalized')

draw_3d(fd0_31, nFile0, 'High Leak - Node Pressure Deviation Normalized')
...
def draw_2d_flow(graphArr):
    ...
    lc = nx.draw_networkx_labels(G, pos = mid_post_dict0, labels = labels, font_size=36, font_color='r')

    plt.colorbar(nc)
    plt.axis('off')
    plt.savefig("/Users/aya/Documents/code-pfs/gas-nx/plots/03302020--latestVel/flow2d_pipes_21_zerDeg.png")
    plt.show()
